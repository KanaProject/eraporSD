...
<body class="bg-gradient-to-br from-primary-900 via-primary-800 to-primary-600 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-2xl overflow-hidden animate-modal">
        ...
        <div class="p-8">
            <div class="text-center mb-6">
                <div class="w-14 h-14 rounded-full bg-primary-50 flex items-center justify-center mx-auto mb-3">
                    <span class="text-primary-700 font-extrabold text-xl">{{ substr($user->name, 0, 1) }}</span>
                </div>
                <h2 class="text-lg font-bold text-slate-800">Halo, {{ $user->name }}!</h2>
                <p class="text-slate-500 text-sm mt-1">
                    Anda memiliki <span class="font-semibold text-primary-700">2 peran aktif</span>. Silakan pilih bagaimana Anda ingin masuk:
                </p>
            </div>
            ...
            <div class="space-y-3">
                ...
                <form method="POST" action="{{ route('role.select.post') }}">
                    ...
                    <button type="submit" class="w-full group flex items-center gap-4 p-4 border-2 border-slate-200 rounded-xl hover:border-primary-400 hover:bg-primary-50 transition-all duration-200 text-left">
                        ...
                        <div class="flex-1">
                            <div class="font-semibold text-slate-800 group-hover:text-primary-700 transition-colors">Masuk sebagai Guru</div>
                            <div class="text-sm text-slate-500">Input nilai mata pelajaran yang Anda ampu</div>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-slate-300 group-hover:text-primary-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <form method="POST" action="{{ route('role.select.post') }}">
                    ...
                    <button type="submit" class="w-full group flex items-center gap-4 p-4 border-2 border-slate-200 rounded-xl hover:border-primary-400 hover:bg-primary-50 transition-all duration-200 text-left">
                        ...
                        <div class="flex-1">
                            <div class="font-semibold text-slate-800 group-hover:text-primary-700 transition-colors">Masuk sebagai Wali Kelas</div>
