<x-layouts.guru title="Dashboard Guru">
    ...
    <div class="bg-gradient-to-r from-blue-900 to-blue-700 rounded-xl shadow-lg p-6 mb-6 text-white flex flex-col md:flex-row items-center md:justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-lg md:text-2xl font-bold truncate">{{ Auth::user()->name }}</h2>
            </div>
        </div>
    ...
    <!-- Empty State -->
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-12 text-center flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <h2 class="text-lg md:text-2xl font-bold truncate text-slate-800 mb-2">Belum Ada Mata Pelajaran yang Diampu</h2>
        <p class="text-slate-500 max-w-md text-base leading-relaxed mb-8">
            Saat ini Anda belum ditugaskan untuk mengajar mata pelajaran apa pun pada tahun ajaran ini. Silakan hubungi tim Kurikulum untuk melakukan pembagian tugas mengajar.
        </p>
        <a href="https://wa.me/" target="_blank" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium transition-colors shadow-sm">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12.76c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.076-4.076a1.526 1.526 0 011.037-.443 48.282 48.282 0 005.68-.494c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
            </svg>
            Hubungi Tim Kurikulum
        </a>
    </div>
    @else
    <!-- Widgets -->
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
        ...
        <div class="bg-gradient-to-br from-emerald-600 to-green-700 rounded-xl shadow-md p-3 md:p-4 text-white flex items-center gap-3 md:gap-4 overflow-hidden transition-transform hover:scale-[1.02]">
            ...
            <div>
                <p class="text-xs md:text-sm text-green-100 font-medium truncate">Kelas Saya</p>
                <h3 class="text-base md:text-xl font-bold truncate">{{ $classesTaught->count() }} Kelas</h3>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl shadow-md p-3 md:p-4 text-white flex items-center gap-3 md:gap-4 overflow-hidden transition-transform hover:scale-[1.02]">
            ...
            <div>
                ...
                <h3 class="text-base md:text-xl font-bold truncate truncate max-w-[140px]" title="{{ $subjectsTaught->count() > 1 ? $subjectsTaught->count() . ' Mapel' : ($subjectsTaught->first()?->name ?? '-') }}">
        ...
        <div class="bg-gradient-to-br from-indigo-500 to-purple-700 rounded-xl shadow-md p-3 md:p-4 text-white flex items-center gap-3 md:gap-4 overflow-hidden transition-transform hover:scale-[1.02]">
            ...
            <div>
                <p class="text-xs md:text-sm text-indigo-100 font-medium truncate">Semester</p>
                <h3 class="text-base md:text-xl font-bold truncate">{{ $activePeriod ? $activePeriod->name : '-' }}</h3>
            </div>
        </div>
